...
so I have chosen to do the same. Likewise the same applies for `containerisation'.}
 will be a very useful technology for the testbed, since it will allow us to model
...
has some weaknesses; first of all if they wish to change their earlier steps
...
in a `cloud image', usually available directly from the operating system 
maintainers\footnote{For example \url{https://cloud-images.ubuntu.com/}}.
On the first boot of the image \emph{cloud-init} will check for supported data sources,
(such as from a list of supported URLs) from which to obtain user data and instance metadata.
...
\begin{singlespace}
	\begin{enumerate}
		...
		\item The disk is attached to the virtual machine via the \emph{libvirt Domain} XML.
	\end{enumerate}
\end{singlespace}
...
When creating virtual machines \emph{kvm-compose} calls the \texttt{genisoimage} program\footnote{\url{https://linux.die.net/man/1/genisoimage}}
...
The keys are also accessible via the \texttt{cloud-init query ds.meta\_data.*key*} command inside the VM.
The user data is a \texttt{\#cloud-config} file in the YAML format, which when loaded by the \emph{cloud-init}
agent, is put through the \emph{jinja} templating engine, allowing variable substitution from the instance
...
implementations such as \emph{SwissCovid}.
 The SDK also has a demo \texttt{calibration-app} (that uses
...
\texttt{/examples/dp3t} contains a small patch to the source of the \texttt{calibration-app} that
...
traffic can then be collected using the \texttt{ovs-tcpdump} utility of 
...
This creates a temporary mirror port on the specified bridge, with the \texttt{--span}
...
to confirm that the virtual machines were set up. 
...
of the testbed. This is not something I have had time to conduct tests for, but I can theorise that it will
...
It is architectured using the serverless model, and depends on services including \emph{AWS} 
...
cloud services; due to a lack of standardisation and unique cloud service implementations.
This means
...
After switching to the \emph{DP3T} service I was able to set up the backend server within
...
Whilst this does allow the app to launch successfully and data to be collected in the testbed,
...
Nevertheless I tried to set up a \emph{Signal} server in \emph{AWS}, and was able to configure
...
schema of the \emph{DynamoDB} tables. As a result I decided to use v4.97 of the \emph{Signal}
...
generated through the testbed. I consider this objective to have been met, and the process
...
Although Bluetooth is a major part of its function, this does not necessarily have to be a complete barrier
...
which can easily be maintained in a \emph{Git} repository for example,
and then running the \texttt{kvm-compose up} command would produce an identical setup.
The level of automation is
unfortunately less well developed when it comes to mobile apps. 
While in the \emph{DP3T} example
...
applications which have been built
...
virtual machine. Both would have to find a solution for selecting the useful / relevant memory
...
a GUI in order to launch, this could be worked around. 
