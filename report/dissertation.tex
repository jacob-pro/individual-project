...
using the technologies described in chapter \ref{chap:technical}, namely \emph{QEMU/KVM}, \emph{libvirt}, \emph{Open vSwitch}, and the \emph{Rust} language.
...
for virtualization should be enabled in the system, and if needed for the application being tested - 
...
And current directory name is taken to be used as the `project name' (again unless specified with
the \texttt{--project-name} flag).\\ 
...
The prefix is used to reduce the possibility that resources in the current testbed project will collide
...
it is created according to its specification (with the project prefix). 
Files required for the virtual machines to function such as disk images are placed in the current directory.
The \texttt{down} does the opposite in reverse order;
removing the machines (and any left over files) and then the bridges. \\
...
While developing \emph{kvm-compose} I needed to use the \emph{libvirt} library for interacting
with \emph{QEMU/KVM}, as well as the bindings to
 access it from the \emph{Rust} language: \emph{libvirt-rust}~\cite{libvirt_rust}.
