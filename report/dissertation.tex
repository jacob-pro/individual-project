...
the data plane, such as to determine which paths a particular packet should take. For example a
...
use to manage and configure the network device. For example the SSH protocol.
...
\emph{libvirt}, \emph{Open vSwitch}, and the \emph{Rust} language.
...
- name: br0                     # (Bridge will be prefixed with project name)
  ...
  connect_external_interfaces: [eth0]
      # When 'stealing' one of the hosts adapters 
      # you will need to assign the new adapter an IP
...
The currently supported cloud images may be listed using the \texttt{cloud-images} subcommand of \emph{kvm-compose}.
